...
const HciErrors = require('./hci-errors');
...
/** @type {import('../../src/node-ble-host').Scanner} */
...
/**
 * @param {'public' | 'random'} addressType 
 * @param {string} address E.g. `AA:AA:AA:AA:AA:AA`
 */
...
/**
 * 
 * @param {import('hci-socket').default} transport
 * @param {*} staticRandomAddress 
 * @param {*} initCallback 
 */
function BleManager(transport, staticRandomAddress, initCallback) {
	...
	/** @type {import('ble-host').ScannerInternal[]} */
	var scanners = [];
	...
	/** Allows the user to pass in their own connections and pendingConnections objects to be used
	 * by the class, so that for instance the user can trap their set and delete methods to keep
	 * their own index of connections and pendingConnections by device address. Also ensures these
	 * objects are exposed on the manager instance as properties */
	this.useObjects = function (userPendingConnection, userConnections) {
		// Careful, `this` === `global`, not the class instance. Must use `manager` to set/expose instance properties
		pendingConnections = this.pendingConnections = userPendingConnection;
		connections = this.connections = userConnections;
	}

	init(initCallback);
	function init(callback) {
		function fail(commandName, status) {
			callback(new Error(commandName + " failed with status: " + HciErrors.toString(status)));
		}
		...
		const fns = ['reset', 'setGetBDAddr', 'clearLEWhitelist', 'readWhitelistSize', 'setEventMask', 'setLeEventMask', 'readLeBufferSize', 'readBufferSize', 'readLeSupportedStates']
		cont();
		const t0 = Date.now()
		function cont() {
			if (closed) return
			var fn = queue.shift();
			if (fn) {
				// console.log(`init cont() ${transport.name} ${queue.length + 1} steps left, doing ${fns[fns.length - queue.length - 1]}`)
				fn();
			} else {
				initComplete = true;
				initTriesLeft = 2;
				console.log(`${transport.name} init completed in ${Date.now() - t0}ms`)
				...
				ops[i](function(status) {
					if (status != 0) {
						adapter.leClearWhiteList(function(status2) {
							currentWhiteList = [];
							callback(new Error("Couldn't modify white list: " + HciErrors.toString(status)));
						});
					} else {
						cont(i + 1);
					}
	...
	function triggerScannerChange() {
		...
		if (scanEnabled && (!scanners.length || scanningNeedsRestart || stopScanCallback)) {
		...
		if (scanners.length) {
			...
			if (scanParametersChanged || mustUpdateWhiteList) {
				...
				if (scanParametersChanged) {
					scannerCommandInProgress = true;
					// Scanning filter policy can be unfiltered or bdAddr whitelist. There is no scanning filter for service UUIDS in the BT spec.
					adapter.leSetScanParameters(leScanType, scanInterval, scanWindow, ownAddressType == "public" ? 0 : 1, useWhiteList ? 0x01 : 0x00, function(status) {
						scannerCommandInProgress = false;
						if (status != 0) {
							scanningFailed("Could not set scan parameters: " + HciErrors.toString(status));
						} else {
			...
			function contAfterWhiteList() {
				...
				adapter.leSetScanEnable(true, currentScanFilterDuplicates, handleAdvReport, function(status) {
					scannerCommandInProgress = false;
					if (status != 0) {
						scanningFailed("Could not start scan: " + HciErrors.toString(status));
					} else {
						scanEnabled = true;
	...
	function handleAdvReport(eventType, addressType, address, data, rssi) {
        for (const scanner of scanners) {
            // Special case for when we are passive scanning but we have already emitted the initial
            // passive scanning advertisement data with the parsed data, to avoid parsing it all
            // again since all we care about in our app is the rssi from now on
            if (!scanner.activeScan && !scanner.filterDuplicates && scanner.duplicateCache.isDuplicate(addressType + address)) {
                // console.log(`Emitting report-addr for ${address} which is in duplicate cache`)
                return scanner.scanner.emit('report-addr', address, rssi)
            } else if (!scanner.activeScan && !scanner.filterDuplicates) {
                scanner.duplicateCache.add(addressType + address)
            }

			if (scanner.filterDuplicates && scanner.duplicateCache.isDuplicate(addressType + address)) {
				// Saw as low as 37 for onboard Pi Bluetooth on apa-14


				// if (!loggedDup) {
				// 	console.log(`Duplicate: ${address} once duplicateCache reached size ${scanner.duplicateCache.size()}`);
				// 	loggedDup = true
				// }
				return;
			}
	...
	function onConnected(role, aclConn, pendingConn, connectedCallback, peerAddressType, peerAddress, connInterval, connLatency, supervisionTimeout, masterClockAccuracy) {
		...
		Object.defineProperty(conn, 'disconnect', {enumerable: true, configurable: false, writable: false, value: function(reason, startIgnoreIncomingData) {
			switch (reason) {
				case HciErrors.AUTHENTICATION_FAILURE:
				...
				case HciErrors.UNACCEPTABLE_CONNECTION_PARAMETERS:
					break;
				default:
					reason = HciErrors.REMOTE_USER_TERMINATED_CONNECTION;
			}
			if (startIgnoreIncomingData) {
				ignoringIncomingData = true;
			}
			if (disconnectInitiated || disconnected) {
				// Observed the following sequence which seems to have been the Pi Zero telling HCI
				// to disconnect, but it then just dropping the command and staying connected.
				// Similar behaviour has been observed with connect commands, where the library
				// thinks it has sent the connect command and is waiting for its first callback to
				// be invoked but it never actually reaches the adapter or something and so it just
				// hangs in a state with conn.connecting = true and connectCommandInProgress = true 
				
				/* 
				< HCI Command: Disconnect (0x01|0x0006) plen 3                                                                                                       #8146 [hci1] 646.073018
						Handle: 16
						Reason: Remote User Terminated Connection (0x13)
				> ACL Data RX: Handle 16 flags 0x02 dlen 16                                                                                                          #8147 [hci1] 649.490780
					LE L2CAP: Connection Parameter Update Request (0x12) ident 82 len 8
						Min interval: 8
						Max interval: 8
						Slave latency: 0
						Timeout multiplier: 300
				< ACL Data TX: Handle 16 flags 0x00 dlen 10                                                                                                          #8148 [hci1] 649.498905
					LE L2CAP: Connection Parameter Update Response (0x13) ident 82 len 2
						Result: Connection Parameters accepted (0x0000)
				> HCI Event: Number of Completed Packets (0x13) plen 5                                                                                               #8149 [hci1] 649.590254
						Num handles: 1
						Handle: 16
						Count: 1
				*/
				...
				console.warn(`disconnectInitiated or disconnected is already true. Calling disconnect again`)
			}
	...
	function triggerConnectionChange() {
		...
		var pendingConnectionsArr = Object.values(pendingConnections);
		...
		// Disabled this since it triggers the incomplete/incorrect `startScan` call below
        var mustUseScan = false // pendingConnectionsArr.length > whiteListSize || connParams == null; // TODO: or if scanning and can't scan and connect concurrently
					...
					adapter.leCreateConnection(48, 48, 0x00, conn.bdAddrType == "public" ? 0x00 : 0x01, connParams.bdAddr, ownAddressType == 'public' ? 0x00 : 0x01, connParams.connIntervalMin, connParams.connIntervalMax, connParams.connLatency, connParams.supervisionTimeout, connParams.minimumCELength, connParams.maximumCELength, status => {
						...
						if (status == 0) {
							...
							timeoutClearFn = manager.setTimeout(() => {
								cancelCommandInProgress = true;
								adapter.leCreateConnectionCancel(status => {
									// If we cancelled in time, the connection complete event will be sent with UNKNOWN_CONNECTION_IDENTIFIER, otherwise the connection completed as we really wanted
								});
							}, 20000);
						} else {
							// Probably max connections reached, possibly failed because controller can't put it into initiating state at this time
						...
						if (status == HciErrors.UNKNOWN_CONNECTION_IDENTIFIER) {
	...
	this.startScan = function(parameters) {
		...
		/** @type {import('../../src/node-ble-host').ScannerInternal} */
		var scanner = {
			id: idGenerator.next(),
			scanner: new Scanner(),
			...
			duplicateCache: new DuplicateCache(1024),
		...
		scanner.scanner.stopScan = function() {
			if (!scanner.stopped) {
				...
				scanners = scanners.filter(s => s.id != scanner.id);
				triggerScannerChange();
			}
		};
	...
	this.connect = function(bdAddrType, bdAddr, parameters, callback) {
		...
		callback = fixCallback(this, callback);
		validate(!Object.values(pendingConnections).some(c => c.peerAddressType == bdAddrType && c.peerAddress == bdAddr.toUpperCase()), `There is already a pending connection to device ${bdAddr}`);
		validate(!Object.values(connections).some(c => c.peerAddressType == bdAddrType && c.peerAddress == bdAddr.toUpperCase()), `There is already an established connection to device ${bdAddr}`);
			...
			if (!connection.cancelled) {
				...
				if (connection.connecting) {
					...
					if (createConnectionIsOutstanding && !cancelCommandInProgress) {
						cancelCommandInProgress = true;
						adapter.leCreateConnectionCancel(status => {
							// This cb is called 4-10ms after the command is issued, however if this
							// adapter has seen an advertisement it will both already be connecting
							// to the device when leCreateConnectionCancel is called and we should
							// expect to get COMMAND_DISALLOWED
							...
							// if (status === HciErrors.COMMAND_DISALLOWED) {
							// 	console.log(`⚠️  leCreateConnectionCancel returned status COMMAND_DISALLOWED (no HCI_LE_Create_Connection command is pending)`)
							// } else {
							// 	console.log(`status from leCreateConnectionCancel = ${HciErrors.toString(status)}`)
							// }
						});
					}
				} else {
...
module.exports = Object.freeze({
	// Exported so we can grab types from the js class to our .d.ts
	BleManager: BleManager,
	/** @param {(err: null | Error, manager: InstanceType<typeof BleManager>) => void} initCallback */
