...
const { default: HciSocket } = require('hci-socket');
...
/** Precomputed 2 char hex byte strings for 0..255 by index */
const INT_HEX_BYTE = [...Array(256).keys()].map(n => n.toString(16).toUpperCase().padStart(2, '0'))

// This was refactored to a class as assigning each of the this.xxx instance methods every time an
// instance was constructed was fairly expensive
class PacketReader {
	/** @param {Uint8Array} buffer */
	constructor(buffer, throwFn) {
		/** @type {Uint8Array} */
		this.buf = buffer
		/** @type {Function} */
	...
	u32() {
		...
		var v = this.buf[this.pos] | (this.buf[this.pos + 1] << 8) | (this.buf[this.pos + 2] << 16) | (this.buf[this.pos + 3] << 24);
		this.pos += 4;
		return v;
	};
	bdAddr() {
		if (this.pos + 6 > this.buf.length) {
			this.throwFn();
		}
		var str = '';
		for (var i = 5; i >= 0; i--) {
			// >= 5x speedup vs orignal method
			str += INT_HEX_BYTE[this.buf[this.pos + i]];
			if (i != 0) {
				str += ':';
			}
		}
	...
	buffer(length) {
		...
		var v = this.buf.slice(this.pos, this.pos + length);
		this.pos += length;
		return v;
	};
	getRemainingBuffer() {
 ...
 * @param {*} hardwareErrorCallback 
 */
function HciAdapter(transport, hardwareErrorCallback) {
	this.transport = transport
	this.toString = () => `[Object HciAdapter (transport ${transport.name})]`
	...
	/** Used to control sendCommand retries */
	var resolvePendingCommandResp = null
	...
	async function reallySendCommand(opcode, buffer, callback, handle) {
		if (isStopped) {
			return;
		}
		...
		const writeData = Buffer.concat([header, buffer])
		const prom = new Promise(resolve => resolvePendingCommandResp = resolve)
		let attempt = 0
		// Used to be 100, increased to 1000 because when devtools memory profiler attached this would cause a crazy amount of "Retry"
		const WRITE_RETRY_TIMEOUT = 1000
		// If reset fails we handle this separately from our user code by closing the socket,
		// reopening a new one and attempting to create a new BleManager.

		let maxAttempts = opcode === COMMANDS.RESET_CMD ? 1 : 6
		while (++attempt <= maxAttempts) {
			if (isStopped) return console.log(`⚠️  Aborting trying to write ${COMMANDS[opcode]} to ${transport.name}: adapter stopped/transport closed`)
			if (attempt > 1) console.log(`⚠️  Retry ${attempt - 1} sending ${COMMANDS[opcode]} to ${transport.name}`)
			transport.write(writeData)
			const result = await Promise.race([prom, new Promise(resolveTimeout => setTimeout(resolveTimeout, WRITE_RETRY_TIMEOUT))])
			if (result)
				return
		}
		if (isStopped) return console.log(`⚠️  ${maxAttempts} failed attempts to send command ${COMMANDS[opcode]} to ${transport.name}, but the adapter is stopped/transport closed so no error will be thrown`)
		// Dont throw and crash program if we failed to get a response on RESET command. We will
		// handle this from user code
		if (opcode !== COMMANDS.RESET_CMD)
			throw new Error(`${maxAttempts} failed attempts to send command ${COMMANDS[opcode]} to ${transport.name}`)
	}
	...
	function sendCommand(opcode, buffer, callback, handle) {
		...
		// Workaround for bug where disconnect AND cancel connection commands doesn't actually get
		// sent through to HCI/the adapter and we just hang (Pi Zero) - unblock the queue of
		// commands and "reallySend" these commands whenever requested
		if (pendingCommand != null) {
			// console.warn(`Queuing command ${COMMANDS[opcode]} on handle ${handle} because there is already a pendingCommand ${COMMANDS[pendingCommand.opcode]}...`)
	...
	function triggerSendPackets(conn) {
		while (numFreeBuffers != 0) {
			...
			if (!conn) {
				var candidates = [];
				for (handle in activeConnections) {
				...
				var selectedConn = activeConnections[handle];
			} else {
	...
	/** Read the RSSI for an existing connection given its handle */
	...
	function handleLeConnectionComplete(r) {
		...
		if (status == HciErrors.ADVERTISING_TIMEOUT) {
			...
			if (status !== 2)  // Connection cancelled
				console.log(`handleLeConnectionComplete with status != 0 ${status}, connCallback is`, connCallback);
			var cc = connCallback;
			connCallback = null;
			if (cc) {
				cc(status);
			...
			if (!callback)
				console.error(`Callback is not defined in handleLeConnectionComplete, role is ${role == ROLE_MASTER ? 'master' : 'slave'}`)
	...
	function handleLeAdvertisingReport(r) {
		if (scanCallback) {
			...
			for (var i = 0; i < numReports && r.getRemainingSize() > 0; i++) {
				var eventType = r.u8();
	...
	function onData(data) {
		/** Invalid length or malformed packets are often indicative of issues in the BLE packet
		 * stream. These issues might arise from corrupted BLE packets possibly intended for another
		 * recipient, signal interference, hardware malfunctions, or even software bugs in the BLE
		 * stack or the application itself. Instead of crashing the application, we log a detailed
		 * warning to aid in diagnosing these issues. Understanding the context (like packet type
		 * and connection handle) and the packet content can be crucial for pinpointing the source
		 * of these errors. */
		const throwInvalidLength = () => {
			try {
				// Determine packet type based on the first byte of the data
				const packetType = data.length > 0 ? data.readUInt8(0) : "unknown"
				let packetTypeName
				switch (packetType) {
					case 0x01: packetTypeName = "HCI Command Packet"; break
					case 0x02: packetTypeName = "ACL Data Packet"; break
					case 0x04: packetTypeName = "HCI Event Packet"; break
					default: packetTypeName = "Unknown Packet Type"
				}
	
				// Connection handle and expected length are specific to ACL Data Packets
				let connectionHandle
				let expectedLength
				if (packetType === 0x02 && data.length >= 5) { // ACL Data Packet header is 5 bytes
					connectionHandle = data.readUInt16LE(1) & 0x0FFF // Extracting Connection Handle
					expectedLength = data.readUInt16LE(3) // ACL Data Packet payload length
				}
	
				// Log the detailed information
				console.log(`⚠️ Invalid length or malformed packet detected on transport ${transport.name}`)
				console.log(`Packet Type: ${packetTypeName} (${packetType})`)
				if (connectionHandle !== undefined) {
					console.log(`Connection Handle: ${connectionHandle}`)
					console.log(`Expected Length: ${expectedLength}`)
				}
				console.log(`Actual Length: ${data.length}`)
				console.log(`Packet Data (Hex): ${data.toString('hex')}`)
				if (pendingCommand) {
					console.log(`Pending Command: ${pendingCommand.opcode}`)
				} else {
					console.log('No pending command.')
				}
			} catch (error) {
				console.log(`Error while logging invalid length or malformed packet:`, error)
			}
		}
		if (data.length == 0) {
			throwInvalidLength();
		...
		if (packetType == HCI_EVENT_PKT) {
			...
			if (eventCode == EVT_CMD_COMPLETE || eventCode == EVT_CMD_STATUS) {
				...
				var numPkts = r.u8();
				// Dont try to read opcode if there isn't a pending command - also avoids
				// intermittent crashes where adapter sends through these events with
				// malformed/wrong length opcode while not actually connected or doing anything
				if (pendingCommand == null || pendingCommand.opcode != r.u16() /* opcode */) {
					...
					pendingCommand = null;
					resolvePendingCommandResp?.(true)
					resolvePendingCommandResp = null
	...
	this.stop = function() {
		// console.log(`stop on adapter ${this.transport.name} called from`, new Error().stack)
		if (isStopped) {
			return;
		}
		isStopped = true;
		transport.removeListener('data', onData);
		// Keep name on the stubbed transport incase we need to print it for debugging purposes
		const name = transport.name
		transport = {
			name: transport.name,
			write: function(data) {
				throw new Error(`Write to adapter after transport ${name} closed`)
			}
		};
	};
