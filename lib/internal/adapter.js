...
function HciAdapter(transport, hardwareErrorCallback) {
	...
	function onData(data) {
		const throwInvalidLength = () => {
			// Usually these seem to be caused by corrupted BLE packets (maybe intended for another
			// recipient in some cases) so we will just quietly raise a warning instead of crashing
			// unless there is a pending command
			console.warn(`Invalid length or malformed packet on transport ${transport.name}: ${data.toString('hex')}`, pendingCommand)
		}
		if (data.length == 0) {
			throwInvalidLength();
		}
