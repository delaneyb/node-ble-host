							...
							if (typeof fn === 'function') {
								...
								fn.call(obj.userObj, connection, function(err, value) {
									...
									if (!err) {
										...
										validate(value instanceof Buffer, 'Invalid attribute value');
                                        validate(value.length <= obj.maxLength, `The supplied value exceeds the maximum length for this value. valueLength: ${value.length},  maxLength: ${obj.maxLength}`);
...
function AttConnection(attDb, connection, registerOnDataFn, sendDataFn, notifyIndicateCallback, timeoutCallback) {
	...
	registerOnDataFn(data => {
		...
		switch (opcode) {
			case ATT_EXCHANGE_MTU_REQUEST:
				...
				// console.log(`Sending EXCHANGE_MTU_RESPONSE`)
	...
	this.findInformation = function(startingHandle, endingHandle, callback) {
		...
		const t0 = Date.now()
		console.log(`Calling enqueueRequest with buffer containing startingHandle ${startingHandle} endingHandle ${endingHandle}...`)
		enqueueRequest(buffer, function(err, data) {
			console.log(`Got a response in ${Date.now() - t0}ms`)
			if (err) {
				callback(err);
				return true;
			}
			...
			for (var i = 2; i < data.length; i += (format == 0x01 ? 4 : 18)) {
				...
				console.log(`response from request findInformation Got a handle ${handle} with uuid ${uuid}. pushing to list which will be given back to`)
...
function GattConnection(connection, attDb, registerOnDataFn, sendDataFn, registerOnBondedFn) {
	...
	function storeGattCache() {
		// This whole thing is just a check for whether we SHOULD cache the gatt services for this
		// gatt connection
		if (!connection.smp.isBonded) {
			// 0x1801 is a service that defines whether service and characteristic information CAN be cached?
			...
			gattServiceMap.forEach(s => {
				...
				if (s.value) {
					hasGattService = true;
					// The 0x2a05 characteristic on the 0x1801 service's existence indicates
					// services and characteristics may change so we cant cache them
	...
	function createCharacteristic(handle, properties, valueHandle, uuid, end) {
		var characteristic = {
			...
			discoverAllCharacteristicDescriptors: function(callback) {
				...
				var found = [];
				function next(i) {
					// Get descriptors of all characteristics by creating characteristics from all
					// the handles we find between the start (valueHandle + 1) and end (found when
					// getting characteristic) handles
					att.findInformation(i, end, function (err, /** @type {{ handle: number, uuid: string }[]} */ list) {
						list = list || [];
						var last = i - 1;
						var max = 0;
						list.forEach(v => {
	...
	this.createCharacteristic = createCharacteristic
	...
	function createGattClientService(start, end, uuid, isPrimary) {
		...
		var service = {
			...
			uuid: uuid,
			/** @type {null | import('ble-host').GattClientCharacteristic[]} */
			characteristics: null,
			/** @type {null | import('ble-host').GattClientService[]} */
	...
	this.createGattClientService = createGattClientService
			...
			function next(i, maxCheckedHandle) {
				...
				function nextSubRange(startingHandle) {
					var cb = function(err, list) {
						// This is going to get invoked with the RESULT from "findByTypeValue" which
						// writes the handle range 0x0001-0xffff Primary Service UUID 0xfff0  "ATT
						// Find by type value request". Lets let this run once, copy whatever comes
						// in "list" and see what happens if we just hardcode it and call cb
						// directly without going via findByTypeValue at all next time...
						list = list || [];
					...
					if (uuid) {
						...
						// console.log(`Skipping call to att.findByTypeValue and invoking the callback with the expected response from the lock directly`)
						// Maybe put this in a setTimeout just to keep it consistent with the
						// libraries sync/async execution flow assumptions
